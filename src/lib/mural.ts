...
export async function listMessages(): Promise<MuralMessage[]> {
  ...
  return rows.map((row) => ({ id: row.id, body: row.body, githubId: row.github_id, login: row.login, avatarUrl: row.avatar_url, createdAt: new Date(row.created_at).toISOString() }));
}

export async function createMessage(input: Omit<MuralMessage, "id" | "createdAt">) {
  const db = await database();
  ...
  const row = rows[0];
  return { id: row.id, body: row.body, githubId: row.github_id, login: row.login, avatarUrl: row.avatar_url, createdAt: new Date(row.created_at).toISOString() } satisfies MuralMessage;
}
