...
O mural é aberto pelo item com badge rosa no dock. Para publicar, cada pessoa
deve entrar pelo GitHub; comentários anônimos não são aceitos.

Para ativar a autenticação, crie um OAuth App em GitHub Developer Settings e
configure as variáveis no deploy:

```text
Authorization callback URL (produção): https://frontfabi.dev/api/auth/callback/github
